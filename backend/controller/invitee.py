...
class BaseInvitee:
    ...
    def build_map_dict_advanced(self, row):
        result = {'account_id': row[0], 'username': row[1], 'full_name': row[2], 'event_id': row[3]}
        return result
    ...
    def getAllInviteesByEvent(self, event_id):
        ...
        result_list = []
        for row in invitee_list:
            obj = self.build_map_dict_advanced(row)
            result_list.append(obj)
        return jsonify(result_list), 200
    ...
    def deleteInviteeWithJson(self, json):
        account_id = json['account_id']
        event_id = json['event_id']
        dao = InviteeDAO()
        result = dao.deleteInvitee(account_id, event_id)
