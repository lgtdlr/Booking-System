...
export class EditEventFormContainer extends React.Component {
    constructor(props) {
        super(props);
    ...
    render() {
        ...
        const { eventId, title, description, date, account_id, start_time_id, end_time_id, room_id } = this.state;
        const values = { eventId, title, description, date, account_id, start_time_id, end_time_id, room_id };
...
const Step = ({ step, values, handleChange, next, prev }) => {
    switch (step) {
        case 1:
            console.log(values)
            return <EditEventDetails values={values}
